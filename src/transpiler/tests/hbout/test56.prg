...
PROCEDURE Main()
   LOCAL lDec := .F. AS LOGICAL

   QOut("bare=" + LTrim(Str(GetQty())))
   QOut("ref_ret=" + LTrim(Str(GetQty(@lDec))))
   QOut("ref_out=" + IIF(lDec, "Y", "N"))
RETURN

   /* First (and only) param is by-ref: the `@lDec` call site above
   marks slot 0 as ref in the reftab. */
FUNCTION GetQty( /*@*/lDecimalQty AS LOGICAL ) AS NUMERIC
   lDecimalQty := .T.
RETURN 42
