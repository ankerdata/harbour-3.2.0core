...
CLASS Shelf
   ...
   EXPORTED:
   METHOD Len() INLINE ::StackLen()
   METHOD Size() INLINE ::StackLen()
   METHOD Stow( xItem )
   METHOD StackLen()
...
METHOD Stow( xItem AS USUAL ) AS USUAL CLASS Shelf
   AAdd(::aItems, xItem)
RETURN xItem

METHOD StackLen() AS NUMERIC CLASS Shelf
RETURN Len(::aItems)

PROCEDURE Main()
   LOCAL oShelf := Shelf():New() AS OBJECT
   oShelf:Stow("jar")
   oShelf:Stow("tin")
   QOut("len=" + LTrim(Str(oShelf:Len)) + " size=" + LTrim(Str(oShelf:Size())))
   QOut("bare=" + LTrim(Str(oShelf:StackLen)))
RETURN
