...
CLASS Tally121
   ...
   DATA iCount AS INTEGER INIT 0
   DATA iTotal AS INTEGER
   DATA nFlags AS INTEGER INIT 0
...
METHOD Add121( iStep AS NUMERIC ) AS OBJECT CLASS Tally121
   ::iCount += iStep
   ::iTotal := ::iCount * 2
...
METHOD Count121() AS NUMERIC CLASS Tally121
RETURN ::iCount

METHOD Rename121( cName AS STRING ) AS OBJECT CLASS Tally121
   // a parameter named as the member: `this.` is what tells them apart
   ::cName := cName
RETURN Self

METHOD Spread121( aSteps AS ARRAY ) AS OBJECT CLASS Tally121
...
PROCEDURE Main()
   ...
   LOCAL oTally := Tally121():New() AS OBJECT
   LOCAL iWhole := 7 AS NUMERIC
   LOCAL nPlain := 9 AS NUMERIC
   LOCAL nMask := hb_bitAnd(7, 3) AS NUMERIC
   LOCAL nHalf := FLAG121_BIT AS NUMERIC

   oTally:Add121(2)
   oTally:Add121(3)
   QOut("tally:", oTally:Count121(), oTally:iTotal, oTally:nFlags)

   Bump121(@iWhole)
   Halve121(@nPlain)
   QOut("by ref:", iWhole, nPlain)

   QOut("divided:", Str(Thrice121(7) / 4, 6, 2), Str(hb_bitAnd(7, 6) / 4, 6, 2), Str(oTally:Count121() / 2, 6, 2))
   nMask := nMask / 2
   nHalf := nHalf / 4
   QOut("mask:", Str(nMask, 6, 2), Str(nHalf, 6, 2))

   oTally:iTotal := nPlain
   QOut("member:", oTally:iTotal)

   oTally:Rename121("t")
   oTally:Spread121({1, 2})
   QOut("names:", oTally:Label121(), oTally:iTotal)
   oTally:Relabel121("u")
   QOut("inline:", oTally:Label121())

RETURN

PROCEDURE Bump121( /*@*/nValue AS NUMERIC )
   nValue += 1
RETURN

PROCEDURE Halve121( /*@*/iValue AS NUMERIC )
   iValue := Int(iValue / 2)
RETURN

FUNCTION Thrice121( iValue AS NUMERIC ) AS NUMERIC
   LOCAL iResult := iValue * 3 AS NUMERIC
RETURN iResult

FUNCTION SetFlag121( nFlags AS NUMERIC, lx AS LOGICAL ) AS NUMERIC
