...
PROCEDURE Main()
   LOCAL cStr := "Hello World" AS STRING
   LOCAL hData := {"Alpha" => 1, "Beta" => 2} AS HASH

   QOut("sub_hit=" + IIF("World" $ cStr, "Y", "N"))
   QOut("sub_miss=" + IIF("xyz" $ cStr, "Y", "N"))
   QOut("key_hit=" + IIF("Alpha" $ hData, "Y", "N"))
   QOut("key_miss=" + IIF("Gamma" $ hData, "Y", "N"))
RETURN
