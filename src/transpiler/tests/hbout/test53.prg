...
PROCEDURE Main()
   QOut("result1=" + LTrim(Str(AddPair(2, 3))))
   QOut("result2=" + LTrim(Str(AddPair(10, 20))))
   QOut("result3=" + IIF(IsLargeFlag(150, .T.), "Y", "N"))
   QOut("result4=" + IIF(IsLargeFlag(50, .F.), "Y", "N"))
RETURN
...
FUNCTION AddPair( nA AS NUMERIC, nB AS NUMERIC ) AS NUMERIC
RETURN nA + nB
...
FUNCTION IsLargeFlag( nValue AS NUMERIC, lFlag AS LOGICAL ) AS LOGICAL
