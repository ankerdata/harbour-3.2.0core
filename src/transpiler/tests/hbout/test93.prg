...
CLASS Lantern
   ...
   DATA nLumens AS NUMERIC INIT 800

ENDCLASS

CLASS Sconce

   DATA oLight AS OBJECT
...
METHOD New( oLight AS OBJECT, cRoom AS STRING ) AS OBJECT CLASS Sconce
   ::oLight := oLight
   ::cLabel := cRoom + "/" + LTrim(Str(oLight:nLumens))
RETURN Self

PROCEDURE Main()
   LOCAL oLantern := Lantern():New() AS OBJECT
   LOCAL oSconce := Sconce():New(oLantern, "hall") AS OBJECT
   QOut("label=" + oSconce:cLabel)
RETURN
