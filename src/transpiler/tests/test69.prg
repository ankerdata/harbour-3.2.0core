...
FUNCTION Probe69( xVal )          // USUAL by-ref output
   xVal := xVal + 1
   RETURN xVal > 0

PROCEDURE Main()
   LOCAL nN := 1                // numeric -> mismatches `ref dynamic`, shimmed
   IF Probe69( @nN )              // hoisted: outer block shims @nN
      Probe69( @nN )              // nested statement: inner block shims @nN
   ENDIF
   ? "nN=" + Str( nN, 2 )
RETURN
