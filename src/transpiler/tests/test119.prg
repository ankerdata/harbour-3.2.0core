...
PROCEDURE Main()
   ...
   ? "set:", nFlags, HasBit119( nFlags )
   nFlags := SetBit119( nFlags, .F. )
   ? "cleared:", nFlags, HasBit119( nFlags )
   ? "word:", Word119( .T. ), Word119( .F. )
   ? "maybe:", Maybe119( .T. ), Maybe119( .F. ) == NIL
   ? "sum:", SetBit119( 0, .T. ) + Half119( .T. ) + Half119( .F. )

   RETURN

FUNCTION SetBit119( nFlags, lx )
   RETURN IIF( lx, hb_bitOr( nFlags, FLAG119_BIT ), hb_bitAnd( nFlags, hb_bitNot( FLAG119_BIT ) ) )

FUNCTION HasBit119( nFlags )
   RETURN hb_bitAnd( nFlags, FLAG119_BIT ) != 0

FUNCTION Word119( lx )
   RETURN IIF( lx, "on", "off" )

FUNCTION Maybe119( lx )
   RETURN IIF( lx, 1, NIL )
