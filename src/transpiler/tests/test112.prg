...
PROCEDURE Main()
   ...
   LOCAL aNums := { 1, 2, 3, 4 }
   LOCAL oBuf := SaleBuffer():New()

   hb_ADel( aNums, 2, .T. )
   ? Len( aNums ), aNums[ 2 ]
   hb_AIns( aNums, 1, 9, .T. )
   ? Len( aNums ), aNums[ 1 ], aNums[ 4 ]
   hb_ADel( aNums, 1 )
   ? Len( aNums ), ValType( aNums[ 4 ] )

   oBuf:aLines := { "a", "b", "c" }
   hb_AIns( oBuf:aLines, 2, "x", .T. )
   ? Len( oBuf:aLines ), oBuf:aLines[ 2 ], oBuf:aLines[ 4 ]
   hb_ADel( oBuf:aLines, 1, .T. )
   ? Len( oBuf:aLines ), oBuf:aLines[ 1 ]

   oBuf:xQueue := { 10, 20 }
   hb_ADel( oBuf:xQueue, 1, .T. )
   ? Len( oBuf:xQueue ), oBuf:xQueue[ 1 ]
   hb_AIns( oBuf:xQueue, 2, 30, .T. )
   ? Len( oBuf:xQueue ), oBuf:xQueue[ 2 ]
