...
| [`hbreftab.c`](hbreftab.c) + `hbreftab.tab` | User-defined function signature table + class member registry + INHERIT edges. Generated by `-GF`. |
| [`hbvartypes.c`](hbvartypes.c) | `--var-types` hint table (non-Hungarian names → types)             |
| **Project maps**                  |                                                                          |
...
**Class rows.** The same table carries per-class information keyed
`Class::…`:

```
ATrust::ATrust__GenerateInitKey  -  NUMERIC  2  oTransaction:OBJECT:-  oPOSStatus:OBJECT:-
BrowseDialog::nBrowseType        -  INTEGER  0
POSStatus::oClerk                -  Clerk    0
```

- `Class::Class__Method` rows are method signatures (same columns as
  functions).
- `Class::member` rows are the **member registry** — every `VAR x AS
  <type>` declared in a CLASS, registered in scan Pass 0. SEND
  inference and the emitter consult them for class-typed receivers,
  walking the INHERIT chain (recorded as `I=<Parent>` tail rows), so a
...
**Convergence.** One `-GF` pass propagates a class-typed parameter
one hop along the call chain (`ecr.prg` → `ECRLoop` → `ECRTran`), so
a real codebase needs the whole loop repeated until the table stops
changing. The driver pipeline (`easipos-transpiled/scripts/scan.py`)
does exactly that — re-runs the loop until the reftab's md5 is stable,
typically 5 passes on EasiPOS (~3.5 min for 677 files). File-order
independence within a pass is guaranteed by the three-pass structure
...
Every diagnostic is a `W00nn` line on stderr in the same shape:

```
hbtranspiler: <file>(<line>): warning W0022  Call to 'Foo' passes STRING for parameter 'x' …
```

Scan-phase (`-GF`) warnings are the CI gate — the pipeline refuses to
emit while any is outstanding — so a category is only promoted to a
warning when it is a defect with a definite source fix whose count
can reach zero. Things that are merely *type debt* go to the
`--type-audit` report instead and never gate.
...
| W0029 | scan  | Compound `/=` `%=` `^=` on an integer field — Harbour division is float, result won't fit. (Plain decimal→int writes compile via a `(long)` cast and are audit-only: `ORM-NARROW`) |
...
Between passes 2 and 3 sits **Pass 2.5 — INTEGER candidacy**. A
LOCAL/STATIC whose numeric life is index-shaped (array subscripts,
`HASHN` keys, `FOR` bounds, `Len()` results) or that flows into an
INTEGER sink (an `AS INTEGER` member or an int/long ORM field)
upgrades `NUMERIC` → `INTEGER` (C# `long`; `long` widens to `decimal`
implicitly so every consumer boundary is free). Fractional writes and
compound `/=` `%=` `^=` disqualify; an index-used variable that a
disqualifier forces back to decimal gets **W0026** rather than a
silent demotion. Parameters are not candidates — their type is the
reftab slot, which merges across callers. See
[test78.prg](tests/test78.prg).

**Receivers.** For `a:b:c` the class of the receiver is resolved by
[`hb_astResolveRecvClass`](hbtypes.c): variables via the type
environment, nested chains via the class-typed member registry
(`POSStatus::oClerk` → `Clerk`), depth-capped at 3 and using *only*
...
### ORM def-class typing

EasiPOS tables are defined by `filedefinitions/*def.prg` factories
(`function DepartmentDef(cPath)`) pairing a field array
(`{ "DEPTNO", "N", len, dec }`) with an `shHash` mapping DB fields to
Hungarian accessors (`"DEPTNO" => "nNo"`). `gen_fieldtypes.py` (in
easipos-transpiled) joins them into `fieldtypes.tsv`:
...
| `INTEGER`         | *(inferred, or `AS INTEGER`)* | `long` — the single integral tier; Harbour integers are 64-bit, so there is no `int`/`AS LONG` distinction. Legacy `as int` members mean the same thing. |
| `STRING`          | `cName`                | `string`                             |
| `LOGICAL`         | `lOk`                  | `bool`                               |
...
| `wapi_Sleep(n)`, `sqlite3_bind_int64(p, 1, n)` — a contrib library's function | `HbWin.wapi_Sleep(n)`, `HbSqlit3.sqlite3_bind_int64(p, 1, n)` — routed to the library's class, never HbRuntime (test106, [Contrib libraries](#contrib-libraries)) — unless the program defines the name itself: then, as Harbour's linker has it, the call is to the program's own function (EasiPOS's `FT_Elapsed`, `FileSize`, `Random`; test108) |
...
| 75        | `hb_csTranslateInline` coverage — `iif()`, `NIL`, `<>` inside INLINE / VAR INIT text |
| 76        | Hash key-type inference (`HASH` / `HASHC` / `HASHN`)   |
| 77        | Function-scope STATIC isolation                        |
| 78        | INTEGER inference (Pass 2.5 index / sink candidacy)    |
| 79        | INHERIT-aware type relations in the reftab             |
| 80        | Faithful by-value semantics for un-`@`'d arguments     |
| 81        | Call-site `/*@*/` by-value marker (silences W0020)     |
| 82        | Numeric `#define` type tiering (`long` / `decimal`)     |
...
references to qualified `<Name>Const` class members, and tells the
type inferencer what type a `#define` reference carries. The path
points at a four-column TSV:

```
NAME<TAB>ClassName<TAB>[OwnerBasename]<TAB>type
ACCEPTANCE   RmCommConst   ``     long
RGBMAX       ColorsConst   ``     decimal
LOCAL_FLAG   MyPrgPrgConst myprg  long
```

- **type** is the C# type of the emitted `const`: any integer-valued
  define is `long` (enum-like ids, flags, bit masks — cast-free at
  subscripts and `HASHN` keys), a fractional one `decimal`, and
  string defines are omitted from the column. Inference reads it, so a
  `#define`-vs-member type disagreement surfaces as W0022/W0024
  instead of a downstream CS error. See [test82.prg](tests/test82.prg).
...
- **`-GF` must be looped to convergence** on a real codebase (see
